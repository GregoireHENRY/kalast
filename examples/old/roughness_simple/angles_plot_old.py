...
# cmap = matplotlib.colors.LinearSegmentedColormap.from_list("newdiv", colors)
# norm = kalast.plot.util.MidPointLogNorm(vmin=0.1, vmax=10, midpoint=1)

# mappable = matplotlib.cm.ScalarMappable(cmap=cmap, norm=norm)
mappable = matplotlib.cm.ScalarMappable(cmap=cmap)

# 0 168 226
colors = mappable.to_rgba(rad_all[226, :])
kalast.plot.util.smap(mesh, colors, label="Radiance (W/m2/sr)", mappable=mappable)
