...
cam = sc.tiri
bod = kalast.spice_entities.moon
...
tmp_all = numpy.load("work/tpm/tmp_all.npy")
...
for jj in range(0, cam.px[1]):
    for kk in range(0, cam.px[0]):
        ...
        try:
            ...
            pxv[jj, kk, 4] = pha * DPR
            ds = [
                kalast.util.distance_haversine(bod.diameter, lo, la, sph_[0], sph_[1])
                for sph_ in sph
            ]
            jj2 = numpy.argmin(ds)
