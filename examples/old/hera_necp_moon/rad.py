...
spec_emi = numpy.load("work/scene/spec_emi.npy")
...
# setup_6d = scipy.io.loadmat("work/rad/setup_6d.mat")
# df = scipy.io.loadmat("work/rad/R_6d.mat")
# R_6d = df["R"]
# ii_gamma = 8
# ii_craterdens = 2
...
for it, image in enumerate(images):
    ...
    for iif in range(0, nf):
        ...
        dau_sun = d_sun / AU
        u_sun = v_sun / d_sun
        cosi = kalast.astro.cosine_incidence(u_sun, n_)

        v_sc = po[it] - p_
        d_sc = numpy.linalg.norm(v_sc)
        u_sc = v_sc / d_sc
        cose = kalast.astro.cosine_incidence(u_sc, n_)
        ...
        cos_ppha = kalast.astro.cosine_incidence(u_sun_proj, u_sc_proj)

        ster_ = area / (d_sc * d_sc)

        tmp_ = tmp_surf[ii_simu[it], iif]

        # ii6d_sun = numpy.argmin(numpy.abs(setup_6d["sun_all"] - numpy.acos(cosi)))
        # ii6d_emi = numpy.argmin(numpy.abs(setup_6d["det_all"] - numpy.acos(cose)))
        # ii6d_ppha = numpy.argmin(numpy.abs(setup_6d["psi_all"] - numpy.acos(cos_ppha)))
        spec_rad_ = numpy.zeros(wl.size)
        for iiwl, wl_ in enumerate(wl):
            # ii6d_lambda = numpy.argmin(numpy.abs(setup_6d["lambda_all"] - wl_))
            # R_ = R_6d[
            #     ii6d_sun, ii6d_emi, ii6d_ppha, ii_gamma, ii_craterdens, ii6d_lambda
            # ]
            R_ = 1.0
            spec_rad_[iiwl] = (
                kalast.tpm.emit.planck(tmp_, wl_) * spec_emi[iiwl] * cose * R_
            )

        # spec_rad_ = kalast.tpm.core.planck(tmp_, wl) * spec_emi * cose * R_
