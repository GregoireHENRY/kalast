...
for p in (
    ...
    mesh = kalast.mesh.Mesh(p, update_pos=lambda x: x * 1e3)
    nfs.append(len(mesh.facets))
    meshes.append(mesh)

    # mesh = kalast.mesh.Surface(p)
    # mesh.update_positions(lambda x: x * 1e3)
    # nf = mesh.indices.size // 3
    # mesh = mesh.make_flat()
    # precmp = [mesh.facet(ii) for ii in range(nf)]
    # nfs.append(nf)
    # meshes.append(mesh)
    # precmp_meshes.append(precmp)
...
et_impact = spice.str2et(setup.date_impact)
et_start = spice.str2et(setup.date_start)
et_end = spice.str2et(setup.date_end)
...
fit_model = {vary: kalast.util.fourier_series(varx, f=f1, n=20, ss="1")}
print(fit_model)
...
et_jd0 = spice.str2et(read_obs.jd_start)

t_jd = (numpy.array(read_obs.jds) - read_obs.jds[0]) * 86400
t_jd += et_jd0 - et_start
t_jd /= t_fac

mags_eli = numpy.array(read_obs.mags)
mags_eli -= mags_eli.mean()

mags_kal = df["mag3"].copy()
mags_kal_mean = mags_kal.mean()
mags_kal -= mags_kal_mean

mags_kal_fit = mag3_fit_fn.copy()
mags_kal_fit -= mags_kal_mean

fig, ax = pyplot.subplots(1, 1, figsize=(11.0, 3.0))
ax.set_xlabel(f"Elapsed time ({t_lbl})")
ax.set_ylabel("Reduced V-band Apparent Magnitude")
ax.scatter(t, mags_kal, s=10, marker="o", fc="k")
ax.plot(t_fn, mags_kal_fit, lw=1, color="k")
ax.errorbar(
    t_jd,
    mags_eli,
    yerr=read_obs.errs,
