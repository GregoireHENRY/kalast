...
def compute_flux(
    ...
    if oth_mesh is not None:
        ...
        n_from_oth = to_oth @ n
        ray_from_oth = p_from_oth - sun_from_oth

        # If ray is parallel to facet normal, no interception test
        # and, If ray is not facing facet normal, no interception test
        det = -ray_from_oth @ n_from_oth
        if (det < -THRESHOLD_PARALLEl or det > THRESHOLD_PARALLEl) and det < 0.0:
            out = oth_mesh.intersect(sun_from_oth, ray_from_oth)
            if out is not None:
                return 0.0, 1
    ...
    return calculation_flux(rbi, albedo, cosi, cose, area, dau_sun, d_obs), 0
...
while True:
    ...
    for iib in [0, 1]:
        ...
        # We compute when body iib is eclipsed by body iib2.
        # We will iterate on each facet of iib and check if any of them is eclipsed by the whole mesh of iib2.
        # So we ask position of iib in fixed frame of iib2 to avoid rotating the whole mesh of iib2.
        # We also ask the rotation to put iib in iib2 fixed frame to positionate every facets of iib in iib2 fixed frame.
        # We need the position of the Sun in fixed frame of iib2 and compute ray to each facet of iib.
        ...
        (sun_from_oth, _lt) = spice.spkpos(
            "sun", et, bods[iib2].frame, "none", bods[iib2].name
        )
        sun_from_oth *= 1e3
        ...
        # dau_obs, same few lines below
        mag = apparent_reduced_vmag(sum_flux, dau_sun * d_obs)

        fluxes[iib].append(sum_flux)
        mags[iib].append(mag)

        d_sun = numpy.linalg.norm(sun_from_oth)
        dau_sun = d_sun / kalast.util.AU
    ...
    mag = apparent_reduced_vmag(flux, dau_sun * d_obs)
