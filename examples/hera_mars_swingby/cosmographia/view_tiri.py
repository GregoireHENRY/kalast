#
# Cosmographia: HERA TIRI at the start of the kalast / shapeviewer window.
#
# Puts the camera just behind the spacecraft on the HERA_TIRI boresight and
...
#
# TIRI field of view: 13.31 x 10.00 deg (1024x768)
#
# Run:  File > Run Script (SHIFT-CMD-R), or  Cosmographia -p view_tiri.py
#
...
SENSOR_NAME = "sensor_HERA_TIRI-MARS.json"
...
cosmo.displayNote( "HERA TIRI  -  " + EPOCH, 3 )
...
cosmo.showObject( "HERA_TIRI-SENSOR" )
...
# --- camera: behind the instrument, looking along +Z of HERA_TIRI ------
# vec1 = position in the frame, vec2 = view direction, vec3 = up.
try:
    cosmo.moveToPovSpiceFrame( "HERA", "HERA_TIRI",
                               [ 0.0, 0.0, -STANDOFF ],
                               [ 0.0, 0.0,  1.0 ],
                               [ 0.0, 1.0,  0.0 ], 0 )
except AttributeError:
...
cosmo.displayNote( "TIRI boresight, frustum end-on, Mars 91,364 km ahead", 6 )
