...
data = [
    ...
    kalast.plot.tool.Data(x=x2, y=d2[0][:, 0], label="heat1d", ls="dashed"),
    # kalast.plot.daily.Data(x=x2, y=d3[0][:, 0], label="multiheats", ls="dotted"),
    kalast.plot.tool.Data(x=x2, y=d4[0][:, 0], label="schorghofer", ls="dotted"),
]
...
data = [
    kalast.plot.tool.Data(x=d["tmp-cols"][0].mean(axis=0), y=y, label="kalast"),
    kalast.plot.tool.Data(x=d["tmp-cols"][0].min(axis=0), y=y),
    kalast.plot.tool.Data(x=d["tmp-cols"][0].max(axis=0), y=y),
    kalast.plot.tool.Data(x=d2[0].mean(axis=0), y=y, label="heat1d", ls="dashed"),
    kalast.plot.tool.Data(x=d2[0].min(axis=0), y=y, ls="dashed"),
    kalast.plot.tool.Data(x=d2[0].max(axis=0), y=y, ls="dashed"),
    kalast.plot.tool.Data(x=d4[0].mean(axis=0), y=y2, label="schorghofer", ls="dotted"),
    kalast.plot.tool.Data(x=d4[0].min(axis=0), y=y2, ls="dotted"),
    kalast.plot.tool.Data(x=d4[0].max(axis=0), y=y2, ls="dotted"),
]

cfg_depth = kalast.plot.tool.Config(data=data)
...
kalast.plot.tool.plot(cfg_depth)
