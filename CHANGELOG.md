...
- The UI app's toolbar shows the iteration and the frame rate, and the capped it/s only while `rate_limited` is on -- the one time the two differ. The `it/s` figure is gone from the default otherwise (`app.config.toolbar` still accepts `{its}`), and the iteration count is gone from the right panel's Run section, where it duplicated the toolbar's.
