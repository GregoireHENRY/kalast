...
- **A paused frame still draws.** `step()` returns `True` as usual — the
  ...
  itself. Under `state.rate_limited` nothing is held: the frame itself waits
  for its turn, so `step()` simply returns at the cap's pace.
...
`KALAST_TIMING=1` prints each load's phases. `flatten=` was the argument until 17 September, default off, so every
