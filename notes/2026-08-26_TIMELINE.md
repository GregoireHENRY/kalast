...
- **Synodic-phase table for the mutual view factors.** The one real
  optimisation left in that path: ~30 sets precomputed over one 2.821 h
  synodic period replace every rebuild in a run of any length, taking a
  1,309-step segment from ~26 min to about 4. Geometry measured to recur to
  5.7 deg, inside the 12 deg cadence already accepted. `2026-08-31_view_factors/`.
- **Insolation on the GPU.** The last thing holding the step loop on the CPU:
  at 3.1M facets it is 60 % of a step, and moving it takes the spin-up from
  20.9 h to roughly 7.8 h. `2026-09-01_gpu_tpm/`.
...
- **14 commits already pushed to origin carry a `Co-Authored-By: Claude`
  trailer.** The 12 unpushed ones were stripped on 1 September; removing it
  from the pushed ones means rewriting published history and force-pushing,
