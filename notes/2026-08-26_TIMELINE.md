...
- **Per-body shadow frusta belong in kalast, not in example scripts.** Agreed
  and deferred. `tiri_deimos_frame.py` and `tiri_deimos_movie.py` hand-roll a
  two-pass render with per-body frusta, faking the bounds by parking the other
  body at `z = -1e9`. The auto-fit fits once to the whole scene, so Mars in
  frame leaves 6 km Deimos almost no shadow-map resolution. Examples should
  stay short; this is engine work.
