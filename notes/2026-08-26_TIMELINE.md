...
- **A "second eclipse" that was filename sorting.** The diffuse frames
  appeared to show an extra eclipse of the secondary that the geometry does
  not contain. Not a frustum or self-shadowing bug: sampling the frames at
  Dimorphos's predicted pixel position found zero dark frames outside the
  umbra windows, and a one-minute geometric scan found only two passages.
  `FrameExporter` wrote unpadded `{N}.png`, so viewers sorted them 0, 1, 10,
  100, ... and scattered the 40 umbra frames into 14 clusters. Now
  `{N:06}.png`; the resume scan still parses old unpadded names.
