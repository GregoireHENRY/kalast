...
- **Rewrote `kalast/tpm/implicit.py`**, which could never have run — its
  ...
  radiative surface boundary is still missing, so it is not yet usable for a
  thermophysical run.
...
Measured constraint that outranks all of it: the TPM loops over facets in
Python, 6.6 ms/step at 3,072 facets and 10.4 ms at 5,040. At 10k facets over
two solar orbits that dominates any solver choice until the loop is
vectorised or moved into Rust.
